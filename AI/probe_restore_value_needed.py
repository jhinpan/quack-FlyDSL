...
def main():
    ...
    result["verdict"] = {
        ...
        "cutedsl_status": (
            ...
            "against quack.rmsnorm._rmsnorm_fwd. THAT REMEDY IS CURRENTLY "
            "UNAVAILABLE, and this field used to imply otherwise. Both "
            "tailscale H200 boxes (hyper00 100.101.70.115, hyper01 "
            "100.105.68.76) have nvidia-cutlass-dsl 4.5.2 installed while "
            "pyproject.toml pins ==4.6.1, and quack/pipeline.py:13 imports "
            "alloc_reserved_mbarrier, which 4.5.2's cutlass.pipeline does not "
            "export (it has MbarrierArray only). So quack.rmsnorm cannot be "
            "imported there either -- a different cause from the MI355X block, "
            "same consequence. Worse, quack/__init__.py:6 gates the whole "
            "cutedsl import chain on torch.version.hip is None, so on a CUDA "
            "box the FlyDSL path is unreachable too: 'import "
            "quack.rmsnorm_flydsl' fails inside quack/__init__.py before it "
            "ever reaches the FlyDSL module. On MI355X the same gate is what "
            "keeps FlyDSL working. Verified on both boxes, including "
            "@CrossVendor's own checkout at 4f36477."
        ),
        "reading": (
