...
cost is committed under `AI/gate_llc_before_after/`).

What is *not* done is re-collection, and the earlier wording for this was
...
So this mode contributes **8 + 1 = 9** contract-invalid cells. The two
full-matrix totals are **37 strict-resident** (the independently computed
47 at-or-under capacity − 10 evicted) and **41 contract-invalid** (37 + the four
`32768x1024` fwd 16-bit cells), neither of them a per-mode count multiplied up.
This paragraph derived 9 for the mode — which *includes* the exact-probed cell —
and then named 37 as the total, which excludes it. 37 is the strict subset;
**41** is the total this section is counting toward. @Reviewer caught the same
premise-vs-total mismatch here that the header already had.
...
> was left reading **6067.802 GB/s**, a stale value, so the sentence that exists
> to name one authoritative number named a superseded one.
...
> tightening the case.
>
> The correction itself then carried a wrong provenance, which @Reviewer also
> caught. I called 6067.802 "a value from the *previous* sidecar". It is not:
> this row reads **6067.802 at `c9c10fc`, 6128.757 at `f4e36e9`, 6391.418 at
> `c0b7c0b`**, so the stale number was two regenerations back and the
> immediately-preceding sidecar said something different again. Getting the
> provenance of a staleness correction wrong is the same defect one level up,
> and it is the third time in this file that a fix has been written from memory
...
5279.90 / 4096.39 = **1.289x**, near the low end of the run range — the
lowest on record is 1.266x (`f4e36e9`), so this is the second-lowest, not the
end point. (The previous
...
1.323x, 1.324x, 1.328x, 1.334x, 1.335x, 1.341x, 1.344x, 1.345x, 1.349x,
1.354x** — fifteen values, one per cell of the table below — so the
...
1. **Fix the `use_evictor` gate first; re-size the evictor second.**
   ...
   recovers the HBM number — the 12 MiB evictor lands **1.962% above** the
   256 MiB one and **2.238% below** the HBM reference (4880.56 / 4786.65 /
   4992.27, recomputed from the committed sidecar).
   These were 1.46% and 0.75%, carried over from a superseded sidecar. The tail
   of this same file already said 2.24%, so the file contradicted itself and
   the stale number was the flattering one — @Reviewer caught it.
...
`1b53896` is disjoint (6018.8–6190.9 vs 6254.4–6453.0), but `742196f` overlaps
on 6089.9–6173.9 and `21e91f6` overlaps on 6373.2–6391.5. One run of three, not
three. @Reviewer caught it. This one is worth flagging beyond the arithmetic:
the claim was refuted by the table immediately above it, in the same section
added to correct a different overclaim — and the correction runs *toward* the
