...
The analytical heuristic already lands on the best width or within 6% of it,
and `waves_per_eu=None` won every forced search. The only two cells with
anything to win are the mid `M=4096` band, where 128 threads per row beats the heuristic's
256 by about a microsecond. So the feature works -- the
search runs, winners persist, a fresh process reloads them -- but the space it
searches has almost nothing in it. Treat `rmsnorm_autotuned` as a hook for
future config axes rather than as a speedup available today.
