...
def _aggregate(profile, controlled) -> dict:
    ...
    for operation in ("fwd", "bwd"):
        ...
        for provider in ("flydsl", "flydsl_tuned"):
            ours_device = (
                device_rows[device_rows["provider"] == provider]
                .set_index(["M", "N"])
                .sort_index()
            )
            ours_public = (
                public_rows[public_rows["provider"] == provider]
                .set_index(["M", "N"])
                .sort_index()
            )
            device_speedups = (
                torch_device["device_us"] / ours_device["device_us"]
            ).tolist()
            public_speedups = (
            ...
            operation_result[provider] = {
                "device_geomean_speedup_vs_torch": math.exp(
                    sum(math.log(value) for value in device_speedups)
                    / len(device_speedups)
                ),
                ...
                "public_geomean_speedup_vs_torch": math.exp(
                    sum(math.log(value) for value in public_speedups)
                    / len(public_speedups)
                ),
...
def main() -> None:
    ...
    if torch.cuda.device_count() != 1:
        raise RuntimeError(
            "expose exactly one idle GPU, for example HIP_VISIBLE_DEVICES=6"
        )
